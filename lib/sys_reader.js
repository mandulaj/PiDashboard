...
SystemInfo.prototype.parseTopCPU = function(line) {
    ...
    var i = 0;
    for(key in this.hardwareStats.CPUstat.time) {
        var value = parseFloat(values[i].slice(0,6)); // remove the name
        this.hardwareStats.CPUstat.time[key] = value; // update the value in hardwareStats
        i++;
    }
...
SystemInfo.prototype.updateHWinfo = function() {
    ...
    else
    {
        ...
        clockMeasure.stdout.on("data", function(data){
            ...
            self.hardwareStats.CPUstat.clockSpeed = data;
        });
        ...
        tempMeasure.stdout.on("data", function(data){
            ...
            self.hardwareStats.GPUstat.temperature = data;
            self.hardwareStats.CPUstat.temperature = data; // TODO: get true CPU data
        });    
    }
    ...
    df.stdout.on("data", function(data){
        ...
        self.hardwareStats.SDstat.fs = filesys;
    });

    // TODO: read this from proc data
    this.hardwareStats.RAMstat.free = os.freemem();
    this.hardwareStats.RAMstat.total = os.totalmem();
}
